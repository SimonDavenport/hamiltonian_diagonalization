...
namespace utilities
{
        ...
        const double tol)               //!<    comparison tolerence
    {
        ...
        if(utilities::is_same<dcmplx, T>::value)
        {
            for(auto it = lhs.m_map.begin(); it != lhs.m_map.end(); ++it)
            {
                ...
                if(abs(rhs.Value(row,col)-lhs.Value(row,col))>tol)  
                {
                    return false;
                }
            }
    ...
    class MappedSparseMatrix
    {
        ...
        //!
        //! Initialize from a dense matrix array
        //!
        void Initialize(
        ...
        //!
        //! Accessor function to read matrix element value (slow)
        //! 
        const T Value(
            const int i,      //!<    row index
            const int j)      //!<    column index
        ...
        //!
        //! Define a function to convert from sparse-mapped to dense format
        //!
        void ConvertToArray(
        ...
        //!
        //! A function to remove any very small elements from the map
        ...
        //!
        //! Print the non-zero matrix elements
        //!
        void Print() const
        {

...
namespace utilities
{
        ...
        const double tol)           //!<    comparison tolerence
    {
        ...
        for(int i=0; i<lhs.GetNbrRows(); ++i)
        {
            ...
            utilities::QuickSort<int, T, _ASCENDING_ORDER_>(&lhs.m_cols[startCol], 
                                                            &lhs.m_data[startCol], endCol-startCol);
            utilities::QuickSort<int, T ,_ASCENDING_ORDER_>(&rhs.m_cols[startCol], 
                                                            &rhs.m_data[startCol], endCol-startCol);
        }
    ...
    class CrsSparseMatrix
    {
        ...
        //!
        //! Clear internal memory allocations
        //!
        void Clear()
        {
            m_cols.clear();
            ...
            const MpiWrapper& mpi)  //!<    Instance of the MPI wrapper class
        {
            ...
            MPI_Status status;
            mpi.Gather<int>(m_rowStarts.data(), nbrRows, m_rowStarts.data(), totalNbrRows,
                            gatherNode, mpi.m_comm, status);
            mpi.Gather<int>(m_cols.data(), nbrNonZeros, m_cols.data(), totalNbrNonZero, 
                            gatherNode, mpi.m_comm, status);
            mpi.Gather<T>(m_data.data(), nbrNonZeros, m_data.data(), totalNbrNonZero, gatherNode, mpi.m_comm, status);
            MPI_Barrier(mpi.m_comm);
        }
            ...
            const MpiWrapper& mpi)              //!<    Instance of the MPI wrapper class
        {
            ...
            if(!m_preparedForParallelMultiply)
            {
                ...
                MPI_Allgather(&nbrRows, 1, mpi.GetType<int>(), &(*dataDistribution)[0], 1, 
                              mpi.GetType<int>(), mpi.m_comm);
                for(int k=0; k<mpi.m_id; ++k)
                {
                ...
                //  Update the m_cols array to shrink the matrix size stored on each node
                for(int i=0; i<nnz; ++i)
                {
        ...
        //!
        //! Return the const address of the start of the m_rowStarts array
        //!
        ...
        //!
        //! Return the address of the start of the m_rowStarts array
        //!     
        int* GetRowStarts()
        {
            return &m_rowStarts[0];
        ...
        //!
        //! Return the const address of the start of the m_cols array
        //!        
        ...
        //!
        //! Return the address of the start of the m_cols array
        //!
        int* GetCols()
        {
            return &m_cols[0];
        ...
        //!
        //! Return the const address of the start of the m_data array
        //!
        ...
        //!
        //! Return the address of the start of the m_data array
        //!
        T* GetData()
        {
            return &m_data[0];

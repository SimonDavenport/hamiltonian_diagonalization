...
#include "../utilities/general/template_tools.hpp"//  For type checking utilities::is_same function
#include "../utilities/general/run_script.hpp"    //  For executing python scripts
...
class HamiltonianBase
{
    ...
    //////      Underlying struct to store class variables          ////////////
    
    HamiltonianData m_data;             //!<    Store Hamiltonian parameters

    //////      Underlying struct to store Fock basis               ////////////
    ...
    //////      Provide infrastructure to support matrix            /////////////
    /////       and eigenvector/eigenvalue storage                  /////////////
        ...
        utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    {
        if(m_data.m_matrixAllocated && mpi.m_nbrProcs > 1)
        {
            ...
            else if(utilities::_DENSE_ == m_data.m_storageMethod)
            {
                ...
                mpi.Gather<T>(m_fullMatrix.data(),m_data.m_nodeDim*m_data.m_fockSpaceDim,m_fullMatrix.data(),m_data.m_fockSpaceDim*m_data.m_fockSpaceDim,0,mpi.m_comm,status);
            ...
            if(0 != mpi.m_id)
            {
                //  Remove the matrix allocation on all nodes except
                //  the master node
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
        ...
        utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
    {
        ...
        //std::cout<<"NODE "<<mpi.m_id<<" totalFalseCount "<<totalFalseCount<<std::endl;
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    ////////////////////////////////////////////////////////////////////////////////
    //! \brief Initialization function to override default construction
    //!
    ////////////////////////////////////////////////////////////////////////////////
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    ////////////////////////////////////////////////////////////////////////////////
    //! \brief Clear all class data structures
    //!
    ////////////////////////////////////////////////////////////////////////////////
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
        ...
        const utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    {
        ...
        //  Sync with the values set on the master node
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    ////////////////////////////////////////////////////////////////////////////////
    //! \brief Set the number of eigenvalues parameter
    //!
    ////////////////////////////////////////////////////////////////////////////////
            ...
            if(m_data.m_matrixAllocated)
            {
                ...
                if(this->MatrixDiagonalCheck(_SERIAL_,mpi))
                {
                    ...
                    for(fock_t i=0;i<m_data.m_fockSpaceDim;++i)
                    {
                        ...
                        //std::cout<<"\tdiagBuffer["<<i<<"] = "<<m_fullMatrix[i*m_data.m_fockSpaceDim+i]<<std::endl;
                    }
                ...
                else
                {
                    ...
                    utilities::linearAlgebra::DiagonalizeSymmetricMatrix<T>(m_fullMatrix.data(),m_eigenvectors.data(),m_eigenvalues.data(),m_data.m_fockSpaceDim,m_data.m_nbrEigenvalues,'U');
        ...
        if(m_data.m_matrixAllocated)
        {
            ...
            if(this->MatrixDiagonalCheck(_PARALLEL_,mpi))
            {
                ...
                for(fock_t i=mpi.m_firstTask;i<=mpi.m_lastTask;++i)
                {
                    ...
                    //std::cout<<"\tdiagBuffer["<<i-mpi.m_firstTask<<"] = "<<m_crsSparseMatrix.Value(i-mpi.m_firstTask,i)<<std::endl;
                }
                ...
                mpi.Gather<double>(diagBuffer,m_data.m_nodeDim,
                mergeBuffer,m_data.m_fockSpaceDim,0,mpi.m_comm,status);
            ...
            else
            {
                ...
                m_crsSparseMatrix.PrepareForParallelMultiplication(&(mv.dataDistribution),
    &(mv.commGroups),&(mv.inputVectorBuffer),&(mv.outputVectorBuffer),mpi);

                //////  CALL MAIN ARPACK ROUTINE

                m_arpackWrapper.DiagonalizeSymmetricMatrix<T>(
                std::bind(&MatrixVectorFunction<T,L1,L2>::Preconditioned,mv,
                std::placeholders::_1,std::placeholders::_2,std::placeholders::_3),
                m_eigenvectors.data(),m_eigenvalues.data(),m_data.m_nodeDim,
                m_data.m_fockSpaceDim,m_data.m_nbrEigenvalues,mpi);
                
                //m_arpackWrapper.DiagonalizeSymmetricMatrix<T>(
                //std::bind(&MatrixVectorFunction<T,L1,L2>::Recalculate,mv,
                //quadraticLookUpTables,quarticLookUpTables,
    ...
    void PrintEigensystem() const
    {
        ...
        if(m_data.m_eigenvectorsCalculated)    
        {
            ...
            for(iSize_t i=0;i<m_data.m_nbrEigenvalues;++i)
            {
                ...
                for(iSize_t j=0;j<eigenvectorNodeDim;++j)
                {
                    std::cout<<m_eigenvectors[i*eigenvectorNodeDim+j]<<"\t";
                }    
                std::cout<<std::endl;
            }
        }
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    //////////////////////////////////////////////////////////////////////////////////
    //! \brief Get the address of the eigenvalue array
    //!
    //////////////////////////////////////////////////////////////////////////////////
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    {   
        ...
        if(writeVectors)     writeValues = writeVectors;
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    //////////////////////////////////////////////////////////////////////////////////
    //! \brief Import all stored eigenvalue and eigenvector data from a text file
        ...
        utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
    {
        ...
        if(0 == mpi.m_id)	// FOR THE MASTER NODE
        {
            ...
            if(readVectors)     readValues = readVectors;
            ...
            if(readValues && readVectors)
            {
                ...
                for(iSize_t i=0;i<m_data.m_nbrEigenvalues;++i)
                {
                    for(fock_t j=0;j<m_data.m_fockSpaceDim;++j,++p_eigs)
                    {
                        ...
                        //std::cout<<"eigenvectorsBuffer["<<i<<"]["<<j<<"] = "<<*p_eigs<<std::endl;
                    }           
                }
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    //////////////////////////////////////////////////////////////////////////////////
    //! \brief Output all stored eigenvalue and eigenvector data to a binary file
    //!
    //////////////////////////////////////////////////////////////////////////////////

...
#include "hamiltonian_base.hpp"                 //  Base class implementation
...
#include "../utilities/mathematics/fermions.hpp"          
                                                //  For binary encoding of 
                                                //  fermion operator commutations 
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
        ...
        utilities::MpiWrapper& mpi)             //!<    Instance of the mpi wrapper class
    {
        ...
        this->m_data.SetDataDimensions(
        this->m_fockBasis.GetReducedFockSpaceDimension(
        ...
        utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
    {
        ...
        if(this->m_data.m_matrixAllocated)
	    {
            ...
            for(int i=lowerRowLimit;i<=upperRowLimit;++i)
            {
                ...
                #if _DEBUG_
                std::cout<<"\tNODE "<<mpi.m_id<<" state = "<<(std::bitset<64>(inState))<<std::endl;
                #endif
                    ...
                    for(iSize_t j=0;j<nbrK1;++j)
                    {
                        ...
                        else
                        {
                            ...
                            if(k1Occupied & ~inState)
                            {
                                ...
                                fock_t outState = utilities::Evaluate_CdC(sign,k1Occupied,k2Occupied,inState,this->m_data.m_highestOrbital);
                                
                                #if _DEBUG_
                                std::cout<<"OUT STATE: "<<(std::bitset<64>(outState))<<std::endl;
                                #endif
                                ...
                                if(0 != sign && outState>=inState)
                                {
                                    ...
                                    if(level1)
                                    {
                                        this->m_fockBasis.FindFockStateIndex(mpi.m_firstTask,TwoLevelFockState(otherState,outState),outIndex);
                                    }
                                    else
                                    {
                                        this->m_fockBasis.FindFockStateIndex(mpi.m_firstTask,TwoLevelFockState(outState,otherState),outIndex);
                                    }
                                    
                                    #if _DEBUG_
                                    std::cout<<"OUT INDEX: "<<outIndex<<std::endl;
                                    #endif
                                }
                            }
                        }
        ...
        utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
    {
	    ...
	    if(this->m_data.m_matrixAllocated)
	    {
            ...
            for(int i=lowerRowLimit;i<=upperRowLimit;++i)
            {
                ...
                #if _DEBUG_
                std::cout<<"\tNODE "<<mpi.m_id<<" mask = "<<(std::bitset<64>(mask))<<std::endl;
                #endif
                        ...
                        if(k3 != k4)        //  For fermions, a squared creation operator gives zero
                        {
                            ...
                            while(tempK2!=0)
                            {
                                ...
                                for(iSize_t j=0;j<nbrK1;++j)
                                {
                                    ...
                                    #if _DEBUG_
                                    std::cout<<"\n\tk1 = "<<(int)k1<<" k2 = "<<(int)k2<<" k3 = "<<(int)k3<<" k4 = "<<(int)k4<<std::endl;
                                    #endif
                                    ...
                                    )
                                    {
                                        ...
                                        int sign;
                                        fock_t outState = utilities::Evaluate_CdCdCC(sign,k1Occupied,k2Occupied,k3Occupied,k4Occupied,inState,this->m_data.m_highestOrbital);
                                        ...
                                        if(0 != sign && outState>=inState)
                                        {
                                            ...
                                            #if _DEBUG_
                                            std::cout<<"\tNODE "<<mpi.m_id<<" in state = "<<(std::bitset<64>(inState))<<std::endl;
                                            std::cout<<"OUT STATE: "<<(std::bitset<64>(outState))<<std::endl;
                                            std::cout<<"OTHER STATE: "<<(std::bitset<64>(otherState))<<std::endl;
                                            std::cout<<"OUT INDEX: "<<outIndex<<std::endl;
                                            #endif
                                            
                                            #if _DEBUG_
                                            PRINT("look up table value",lookUpTables->GetVkkkk(k1,k2,k3,k4));
                                            PRINT("sign",sign);
                                            PAR_PAUSE();
                                            #endif

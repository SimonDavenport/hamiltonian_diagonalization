    ...
    struct Observables
    {
            ...
            utilities::MpiWrapper& mpi)             //!<    Instance of the mpi wrapper class
        {
            if(hamiltonian.m_data.m_eigenvectorsCalculated)
            {
                ...
                MPI_Status status;
                mpi.Reduce(probabilityList, reduceBuffer, hamiltonian.m_data.m_nbrOrbitals*hamiltonian.m_data.m_nbrOrbitals, 
                           0, mpi.m_comm,status);
                ...
                for(iSize_t i = mpi.m_firstTask; i<=mpi.m_lastTask; ++i, ++it_eigs)
                {
                    ...
                    while(tempState!=0) //  Iterate over N terms
                    {
                        ...
                        for(kState_t q = 0; q<functionList.size(); ++q)
                        {
                            ...
                            int sign;
                            fock_t outState = utilities::Evaluate_CdC(sign, k1PlusQOccupied, k1Occupied, inState, 
                                                                      hamiltonian.m_data.m_highestOrbital);
                            if(sign!=0)
                            {
                                ...
                                while(tempState2!=0) //  Iterate over N terms
                                {
                                    ...
                                    //  Act on the current in state with CdC
                                    int sign2;
                                    fock_t outState2 = utilities::Evaluate_CdC(sign2, k2MinusQOccupied, k2Occupied, outState, 
                                                                               hamiltonian.m_data.m_highestOrbital);
            ...
            if(hamiltonian.m_data.m_eigenvectorsCalculated)
            {
                ...
                for(iSize_t e=0; e<hamiltonian.m_data.m_nbrEigenvalues; ++e)
                {
                    ...
                    if(hamiltonian.GetEigenvectorNodeDim()>0)
                    {
                        ...
                        utilities::PartialQuickSort<dcmplx, fock_t, _DESCENDING_ORDER_>(eigsBuffer, fockBuffer, eigenvectorNodeDim, 
                                                                                        nbrStatesPerNode);
                    }
                    ...
                    if(0 == mpi.m_id)    //  FOR THE MASTER NODE 
                    {
                        utilities::PartialQuickSort<dcmplx, fock_t, _DESCENDING_ORDER_>(gatherAmplitudesBuffer, gatherStatesBuffer, 
                                                                                        totalNbrStates, actualNbrStates);

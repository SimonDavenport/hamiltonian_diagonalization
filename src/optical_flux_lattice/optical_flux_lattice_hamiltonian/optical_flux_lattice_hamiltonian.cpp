...
////////////////////////////////////////////////////////////////////////////////
//! \brief Default constructor for the OpticalFluxLatticeHamiltonian class
//! 
//! NOTE: implicitly calls the default constructors of all internal classes
//!
////////////////////////////////////////////////////////////////////////////////

OpticalFluxLatticeHamiltonian::OpticalFluxLatticeHamiltonian()
{}

//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//

////////////////////////////////////////////////////////////////////////////////
//! \brief Constructor for the OpticalFluxLatticeHamiltonian class
//!
...
OpticalFluxLatticeHamiltonian::OpticalFluxLatticeHamiltonian(
    OpticalFluxLatticeHamiltonianData data) //!<    External instance of OpticalFluxLatticeHamiltonianData
    :   m_params(data)
{}
    ...
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    ...
    m_params = OpticalFluxLatticeHamiltonianData(optionList,mpi);

    //  Initialize single particle model data

    m_singleParticleData = SingleParticleParameters(optionList,mpi);
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Destructor for the OpticalFluxLatticeHamiltonian class
//! 
////////////////////////////////////////////////////////////////////////////////
...
void OpticalFluxLatticeHamiltonian::UpdateSqlStatus(
...
void OpticalFluxLatticeHamiltonian::SetSector(
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Generate look-up tables for the quadratic and quartic terms of our
//! Hamiltonian
//!
...
void OpticalFluxLatticeHamiltonian::BuildLookUpTables(
    boost::program_options::variables_map* optionList,
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    ...
    //  Set required parameters from the command line
    ...
    //  Alternatively, look for pre-calculated values stored in some files

    if(m_params.m_blochTableFromFile)
    {
        this->TablesFromFile(mpi);
    }
    else
    {
        //  Allocate memory to store the k-value and reciprocal lattice value tables
        ...
        SingleParticleHamiltonian* blochWaveFunctionTable = new SingleParticleHamiltonian[m_params.m_dimX*m_params.m_dimY]; 
        ...
        SingleParticleHamiltonianArray modelArray;
            
        modelArray.GenerateBlochWaveFunctionTable(cutOff,m_params.m_dimX,m_params.m_dimY,m_params.m_offsetX,m_params.m_offsetY,blochWaveFunctionTable,&m_singleParticleData,mpi);

        this->GenerateQuarticTerms(m_quarticTables.GetVTable(),m_quarticTables.GetDimension(),blochWaveFunctionTable,&gxTable,&gyTable,mpi);
        ...
        do
        {
            ...
            this->GenerateQuarticTerms(&listOfQuarticTerms,m_quarticTables.GetDimension(),blochWaveFunctionTable,&gxTable,&gyTable,mpi);
            ...
            if(0 == mpi.m_id) // FOR THE MASTER NODE
	        {
                ...
                {
                    //std::cout<<"\n\t"<<fabs(abs(*it_list1)-abs(*it_list2))<<std::endl;
                ...
                //  Update the current look-up table
                
                *(m_quarticTables.GetVTable()) = listOfQuarticTerms;
                
                //memcpy(m_quarticTables.GetVTable()->data(),listOfQuarticTerms.data(),m_quarticTables.GetDimension()*sizeof(dcmplx));

                utilities::cout.SecondaryOutput()<<"\n\tBLOCH BAND CUT OFF: "<<cutOff<<"\tMAX CHANGE IN |Vkkkk|: "<<currentTol<<std::endl;
            }
            
            //  sync the currentTol with the master node value
            //  so that the while condition below is valid on all nodes
        ...
        if(0 == mpi.m_id)    // FOR THE MASTER NODE
	    {
            ...
            { 
                *(it_quadraticTable) = blochWaveFunctionTable[i].GetBandEnergy(0);
                ...
                #if _DEBUG_
                std::cout<<"\n\tquadraticTable["<<i<<"] = "<<*(it_quadraticTable)<<std::endl;
                #endif
            }
        }
        ...
        //////      Convert to hash table format now if requested 
        ...
        //////      Store quadratic and quartic term coefficients in some files
        //////      if requested
        
        if(m_params.m_blochTableToFile)
        {
            this->TablesToFile(mpi);
        }
    }
...
void OpticalFluxLatticeHamiltonian::Generate4KTable(
            ...
            for(kState_t k2=0;k2<kMax;++k2,++it_kTable,++it_gxTable,++it_gyTable)
            {
                if(it_kTable < kTable->end() && it_gxTable < gxTable->end() && it_gyTable < gyTable->end())
                {
                    ...
                    *(it_gyTable) = y;
                    ...
                    //std::cout<<"k1 = "<<k1<<" k2 = "<<k2<<" k3 = "<<k3<<" k4 = "<<k4<<std::endl;getchar();
                }
            }
        }
    }
                    ...
                    for(kState_t ky3=0;ky3<m_params.m_dimY;++ky3)
                    {
                        for(kState_t kx2=0;kx2<m_params.m_dimX;++kx2)
                        {
                            for(kState_t ky2=0;ky2<m_params.m_dimY;++ky2)
                            {
                                ...
                                {
                                    kHashTable->Insert(kx1*m_params.m_dimY+ky1,utilities::Key(kx2*m_params.m_dimY+ky2,kx3*m_params.m_dimY+ky3,kx4*m_params.m_dimY+ky4));
                                }
                            }
                        }
                    }
...
void OpticalFluxLatticeHamiltonian::Generate2KTable(
    ...
    const utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
{
    ...
    else
    {
        ...
        for(kState_t k2=0;k2<kMax;++k2)
        {
            //  The list is diagonal due to 2D momentum conservation
...
kState_t OpticalFluxLatticeHamiltonian::FindK1(
...
void OpticalFluxLatticeHamiltonian::GenerateQuarticTerms(
    std::vector<dcmplx>* listOfQuarticTerms,     
    ...
    const
{
    ...
    mpi.DivideTasks(mpi.m_id,dimension,mpi.m_nbrProcs,
    ...
    SingleParticleHamiltonian*  blochTable4 = blochTable;
    dcmplx* p_temp = temp;
    iSize_t counter = 0;
    ...
    {   
        SingleParticleHamiltonian* blochTable3 = blochTable4;
        ...
        {
            SingleParticleHamiltonian*  blochTable2 = blochTable;
            ...
            for(kState_t k2=0;k2<kMax;k2++,++blochTable2,++counter)
            {
                if(mpi.m_firstTask <= (int)counter && mpi.m_lastTask >= (int)counter)
                {
                    ...
                    //std::cout<<blochTable[k1].GetBandEnergy(0)<<std::endl;
                    //std::cout<<blochTable2->GetBandEnergy(0)<<std::endl;
                    //std::cout<<blochTable3->GetBandEnergy(0)<<std::endl;
                    //std::cout<<blochTable4->GetBandEnergy(0)<<std::endl;getchar();
                    ...
                    //std::cout<<blochAup[0]<<std::endl;
                    //std::cout<<blochBdown[0]<<std::endl;
                    //std::cout<<blochCdown[0]<<std::endl;
                    //std::cout<<blochDup[0]<<std::endl;getchar();
                    ...
                    //std::cout<<"\toffsets = "<<offset1<<" "<<offset2<<" "<<offset3<<" "<<offset4<<std::endl;
                    utilities::ArrayConvolution2D<dcmplx,dcmplx,sizeof(dcmplx)>(blochBdown,blochDup,blochTableXDim,blochTableYDim,convolution1,0,0,0,0);
                    utilities::ArrayConvolution2D<dcmplx,dcmplx,sizeof(dcmplx)>(blochCdown,blochAup,blochTableXDim,blochTableYDim,convolution2,offset1,offset2,offset3,offset4);
                    
                    utilities::ArrayConvolution2D<dcmplx,dcmplx,sizeof(dcmplx)>(blochBup,blochDdown,blochTableXDim,blochTableYDim,convolution3,0,0,0,0);
                    utilities::ArrayConvolution2D<dcmplx,dcmplx,sizeof(dcmplx)>(blochCup,blochAdown,blochTableXDim,blochTableYDim,convolution4,offset1,offset2,offset3,offset4);
                    ...
                    {
                        //std::cout<<"\tconvolution1["<<i<<"] = "<<convolution1[i+offset3*convolutionArrayOffset+offset4]<<std::endl;
                        //std::cout<<"\tconvolution2["<<i<<"] = "<<convolution2[i+offset1*convolutionArrayOffset+offset2]<<std::endl;
                        //std::cout<<"\tconvolution3["<<i<<"] = "<<convolution3[i+offset3*convolutionArrayOffset+offset4]<<std::endl;
                        //std::cout<<"\tconvolution4["<<i<<"] = "<<convolution4[i+offset1*convolutionArrayOffset+offset2]<<std::endl;
                    ...
                    *p_temp = Vkkkk.m_sum;
                    ...
                    //std::cout<<"\t"<<counter<<"\t|V_"<<k1<<","<<k2<<","<<k3<<","<<k4<<"| = "<<abs(Vkkkk.m_sum)<<std::endl;
                    //std::cout<<"\t"<<counter<<"\tV_"<<k1<<","<<k2<<","<<k3<<","<<k4<<" = "<<Vkkkk.m_sum<<std::endl;
                    //getchar();
    ...
    //  Gather all data on the master node 

    MPI_Status status;

    mpi.Gather<dcmplx>(temp,taskDim,listOfQuarticTerms->data(),dimension,0,
        mpi.m_comm,status);
...
void OpticalFluxLatticeHamiltonian::ConvertTableFormat(
    const utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class 
{
    if(_ARRAY_ == m_params.m_tableFormat)
    {
        ...
        //  Clear any existing hash table data
        ...
        //  Re-initialize hash table class
        
        //m_hashTables.Initialize(m_params.m_dimX,m_params.m_dimY);
        ...
        //  Update the table store type flag
        
        m_params.m_tableFormat = _HASH_;
        
        //  Mpi synchronize the table values
        
        //m_hashTables.MpiSynchronize(0);
        
        m_quadraticHashTables.MpiSynchronize(0,mpi);
        m_quarticHashTables.MpiSynchronize(0,mpi);
        
        #if _DEBUG_
        m_quadraticHashTables.GetKTable()->Print<kState_t>();
        m_quarticHashTables.GetKTable()->Print<kState_t,kState_t,kState_t>();
        m_quadraticHashTables.GetVTable()->Print<kState_t>();
        m_quarticHashTables.GetVTable()->Print<kState_t,kState_t,kState_t>();
        #endif
    }
...
void OpticalFluxLatticeHamiltonian::TablesToFile(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
	{
    ...
    if(_ARRAY_ == m_params.m_tableFormat)
    {
        m_quarticTables.TableToFile(fileName.str(),mpi);
    }
    else
    {
        int nbrMomentumLabels = 4;
        
        //  Make a temporary copy since TableToFile calls an
        //  mpi gather function and will change the contents 
        //  of the map
        
        QuarticLookUpHashTables temp = m_quarticHashTables;
        
        temp.TableToFile(fileName.str(),nbrMomentumLabels,mpi);
    }
    ...
    if(_ARRAY_ == m_params.m_tableFormat)
    {
        m_quadraticTables.TableToFile(fileName.str(),mpi);
    }
    else
    {
        int nbrMomentumLabels = 2;
    
        //  Make a temporary copy since TableToFile calls an
        //  mpi gather function and will change the contents 
        //  of the map
    
        QuadraticLookUpHashTables temp = m_quadraticHashTables;
...
void OpticalFluxLatticeHamiltonian::TablesFromFile(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
	{
    ...
    if(_HASH_== m_params.m_setTableFormat)
    {
        //  Clear any existing hash table implementation
        ...
        //  update the table storage format flag
    ...
    {
        m_quarticTables.TableFromFile(fileName.str(),mpi);
    }
    else
    {
        int nbrMomentumLabels = 4;
        
        m_quarticHashTables.TableFromFile(fileName.str(),nbrMomentumLabels,mpi);
    }
    ...
    if(_ARRAY_ == m_params.m_setTableFormat)
    {
        m_quadraticTables.TableFromFile(fileName.str(),mpi);
    }
    else
    {
        int nbrMomentumLabels = 2;
    
        m_quadraticHashTables.TableFromFile(fileName.str(),nbrMomentumLabels,mpi);
    }
...
void OpticalFluxLatticeHamiltonian::ChangeToWannierBasis(
    SingleParticleHamiltonian* blochTable,  //!<    Lookup table of Bloch wave functions
    ...
    utilities::MpiWrapper& mpi)             //!<    Instance of the mpi wrapper class
{
...

    {
        ...
        utilities::DiscreteFourierTransform1D(m_params.m_dimX,m_params.m_dimY,fourierCoefficients,outputCoefficients,FFTW_BACKWARD);
        ...
        //  Clear memory allocations used for discrete Fourier transform
...

    {
        ...
        modelArray.GenerateWannierCoefficients(m_params.m_dimX,m_params.m_dimY,m_params.m_offsetX,blochTable,&m_basisChangeMatrix[0],mpi);
                        ...
                        for(kState_t k3y=0;k3y<m_params.m_dimY;++k3y)
                        {
                            ...
                            for(kState_t x2=0;x2<m_params.m_dimX;++x2)
                            {
                                for(kState_t k2y=0;k2y<m_params.m_dimY;++k2y)
                                {
                                    ...
                                    //m_hashTables.GetK1(kRetrieveBuffer,nbrK1,k2,k3,k4);
                                    ...
                                    for(iSize_t j=0;j<nbrK1;++j)
                                    {
                                        ...
                                        //  Update the table of stored values,
                                        //  but only include non-zero values
    ...
    #if _DEBUG_
    //  Some coefficient value checks:

    if(abs(m_quarticHashTables.GetVkkkk(0,1,0,1)-std::conj(m_quarticHashTables.GetVkkkk(1,0,1,0)))<0.000000000001)
    {
    ...
    else
    {
        ...
        std::cout<<"FAILED TEST 1: out by "<<abs(m_quarticHashTables.GetVkkkk(0,1,0,1)-std::conj(m_quarticHashTables.GetVkkkk(1,0,1,0)))<<std::endl;
    ...
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
    {
        m_quadraticHashTables.GetVTable()->Print<kState_t,kState_t>();
    }
    #endif
    ...
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
    {
        m_quarticHashTables.GetVTable()->Print<kState_t,kState_t,kState_t,kState_t>();
    }
    #endif
...
void OpticalFluxLatticeHamiltonian::StoreSigmaZMap(
    SingleParticleHamiltonian* blochTable,  //!<    Lookup table of Bloch wave functions
    const utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
    ...
    //  Reserve memory to store magnetization data
...
void OpticalFluxLatticeHamiltonian::BuildFockBasis(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{ 
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
    {   
    ...
    if(m_params.m_blockDiagonalize)
    {
        ...
        if(m_params.m_useWannierBasis)
        {
            ...
            isZeroDimensional = basis.GenerateFockSpace(m_params.m_nbrParticles,m_params.m_dimX*m_params.m_dimY,std::bind(TestLinearMomentumSector1,std::placeholders::_1,m_params.m_kyTot,m_params.m_dimY),mpi);
        ...
        else
        {
            ...
            isZeroDimensional = basis.GenerateFockSpace(m_params.m_nbrParticles,m_params.m_dimX*m_params.m_dimY,std::bind(TestLinearMomentumSector2,std::placeholders::_1,m_params.m_kxTot,m_params.m_kyTot,m_params.m_dimX,m_params.m_dimY),mpi);
...
void OpticalFluxLatticeHamiltonian::BuildHamiltonian(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    if(m_params.m_lookupTablesBuilt && m_params.m_fockBasisBuilt && !m_params.m_hamiltonianBuilt)
    {
        ...
        //m_hamiltonian.HamiltonianToFile(fileName.str());
    }
    else
    {
...
void OpticalFluxLatticeHamiltonian::Diagonalize(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
...
bool OpticalFluxLatticeHamiltonian::PlotHamiltonian(
    const utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
    const
{       
...
void OpticalFluxLatticeHamiltonian::EigensystemToFile(
...
void OpticalFluxLatticeHamiltonian::EigensystemFromFile(
    ...
    utilities::MpiWrapper& mpi)         //!<    Instance of the mpi wrapper class
{
    if((readEigenvalues || readEigenvectors) && !m_params.m_hamiltonianDiagonalized)
    {
        ...
        if(m_params.m_blockDiagonalize)
        {
            ...
            basis.GenerateFockSpace(m_params.m_nbrParticles,m_params.m_dimX*m_params.m_dimY,std::bind(TestLinearMomentumSector2,std::placeholders::_1,m_params.m_kxTot,m_params.m_kyTot,m_params.m_dimX,m_params.m_dimY),mpi);
...
void OpticalFluxLatticeHamiltonian::StoreHamiltonian(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
    std::stringstream fileName;
...
void OpticalFluxLatticeHamiltonian::RetrieveHamiltonian(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
{
...
void OpticalFluxLatticeHamiltonian::ClearHamiltonian()
{
    if(m_params.m_hamiltonianBuilt)
    {
        m_hamiltonian.Clear();
...
bool OpticalFluxLatticeHamiltonian::CalculateOccupations(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
    const
{
    if(m_params.m_hamiltonianDiagonalized)
...
bool OpticalFluxLatticeHamiltonian::CalculateSusceptibility(
    utilities::MpiWrapper& mpi)       //!<    Instance of the mpi wrapper class
    const
{
...
bool OpticalFluxLatticeHamiltonian::ListMostProbable(
...
bool OpticalFluxLatticeHamiltonian::CalculateDensityDensityFunction(
    utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    const
{
    if(m_params.m_hamiltonianDiagonalized)
...
bool OpticalFluxLatticeHamiltonian::CalculateParticipationRatio(
    utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    const
{
    if(m_params.m_hamiltonianDiagonalized)
...
bool OpticalFluxLatticeHamiltonian::CalculateTranslationalDensityDensityFunction(
    utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    const
{
    if(m_params.m_hamiltonianDiagonalized)
    {
        ...
        for(iSize_t eigenvectorIndex=0;eigenvectorIndex<m_params.m_nbrEigenvaluesToFind;++eigenvectorIndex)
        {
            ...
            observables.TranslationalDensityDensityFunction(
                m_hamiltonian,
                eigenvectorIndex,
                std::bind(StateAddition,std::placeholders::_1,std::placeholders::_2,m_params.m_dimX,m_params.m_dimY,nbrUnitCells*m_params.m_dimX,nbrUnitCells*m_params.m_dimY),
                std::bind(StateSubtraction,std::placeholders::_1,std::placeholders::_2,m_params.m_dimX,m_params.m_dimY,nbrUnitCells*m_params.m_dimX,nbrUnitCells*m_params.m_dimY),
                functionList,mpi);
...
bool OpticalFluxLatticeHamiltonian::CalculateRotationalDensityDensityFunction(
    utilities::MpiWrapper& mpi)   //!<    Instance of the mpi wrapper class
    const
{
    if(m_params.m_hamiltonianDiagonalized)
    {
        ...
        for(iSize_t eigenvectorIndex=0;eigenvectorIndex<m_params.m_nbrEigenvaluesToFind;++eigenvectorIndex)
        {
            ...
            observables.RotationalDensityDensityFunction(
                m_hamiltonian,
                eigenvectorIndex,

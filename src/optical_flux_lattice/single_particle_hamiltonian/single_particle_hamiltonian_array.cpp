...
///////     LIBRARY INCLUSIONS     /////////////////////////////////////////////
#include "single_particle_hamiltonian_array.hpp"
...
////////////////////////////////////////////////////////////////////////////////
//! \brief This function combines a set of 2D indexes into a single index
//!
////////////////////////////////////////////////////////////////////////////////

kState_t SingleParticleHamiltonianArray::CombineIndexes(
...
void SingleParticleHamiltonianArray::GenerateBlochWaveFunctionTable(
    ...
    utilities::MpiWrapper& mpi)             //!<    Instance of the mpi wrapper class
{
    ...
    //  Now generate data to populate the table
    ...
    //  Populate the all matrix elements

    singleParticleHam.PopulateDifferenceMatrix();

    //  No matter what, the Vkkkk can only contain band functions calculated 
    //  on the k-space grid. Let's make a look-up table of such values

    //  Only the diagonal of the single particle Hamiltonian needs updating
    //  for every k state
    ...
    for(iSize_t x=0;x<dimX;++x,kVector-=reciprocalLattice::G2,kVector += increment1)   //  Map out G1 direction
    {
        for(iSize_t y=0;y<dimY;++y,kVector += increment2,++counter)   //  Map out G2 direction
        {
            if(mpi.m_firstTask <= (int)counter && mpi.m_lastTask >= (int)counter)
            {
                ...
                //std::cout<<"NODE: "<<mpi.m_id<<" "<<"kx = "<<kVector.GetKx()<<" ky = "<<kVector.GetKy()<<std::endl;
                //singleParticleHam.PrintDifferenceMatrix();//getchar();
                ...
                //  Include some random phases for testing purposes
                //  (each Bloch vector is defined up to a random phase factor)
                //singleParticleHam.MultiplyByPhase(2.0*PI*(double)rand() / RAND_MAX);
                ...
                //std::cout<<"NODE: "<<mpi.m_id<<" "<<singleParticleHam.GetBandEnergy(0)<<std::endl;
            }
            else
            {
    ...
    for(int node=0;node<mpi.m_nbrProcs;++node)
    {
        ...
        //std::cout<<"NODE: "<<mpi.m_id<<" taskProc = "<<taskProc<<std::endl;

        for(int p=0;p<taskProc;++p,++taskCounter,++p_table)
        {
            //std::cout<<"NODE: "<<mpi.m_id<<" temp["<<taskCounter <<"] = "<<temp[taskCounter ].GetBandEnergy(0)<<std::endl;

            p_table->MpiSynchrnoizedCopy(temp[taskCounter ],node,mpi);
            
            //std::cout<<"NODE: "<<mpi.m_id<<" node = "<<node<<" counter = "<<taskCounter <<std::endl;
        } 
    }
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Numerically calculates the single particle band structure of the 
//! Hamiltonian given in e.g. eq (1) of PRL 109,265301 in a single Brillouin zone
//!
//! NOTE: this function is not designed to be run in parallel 
//!
////////////////////////////////////////////////////////////////////////////////

void SingleParticleHamiltonianArray::CalculateBandstructure(
    ...
    utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
{
    ...
    for(iSize_t x=0;x<dimX;++x,kVector-=reciprocalLattice::G2,kVector += increment1)   //  Map out G1 direction
    {
        for(iSize_t y=0;y<dimY;++y,kVector += increment2,++counter)   //  Map out G2 direction
        {
            ...
            {      
                ...
                //currentHamiltonian->PrintDifferenceMatrix();
                //std::cout<<"kx = "<<kVector.m_kx<<" ky = "<<kVector.m_ky<<std::endl;

                currentHamiltonian->DiagonalizeDifferenceMatrix();

                //std::cout<<currentHamiltonian->GetBandEnergy(0)<<std::endl;
                //std::cout<<currentHamiltonian->GetBandEnergy(1)<<std::endl<<std::endl;
                //getchar();
    ...
    if(getEnergyData)
    {
        ...
        //  Copy the gather buffer back into the array to be returned
...
void SingleParticleHamiltonianArray::PlotBandstructure(
    ...
    utilities::MpiWrapper& mpi)                 //!<    Instance of the mpi wrapper class
{
    ...
    this->CalculateBandstructure(lowerBand,secondBand,bandWidth,bandGap,
    true,&dummy,false,dimX,dimY,offsetX,offsetY,&currentHamiltonian,mpi);
    
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
	{
        //  Output the data as a .dat file
        ...
        if((*optionList)["plot-bandstructure"].as<bool>())
        {
            ...
            //  Wrtie a python script to automatically plot the band structure
...
void SingleParticleHamiltonianArray::PlotBandWidth(
    boost::program_options::variables_map* optionList,     
                                        //!<    Command line arguments
    utilities::MpiWrapper& mpi)         //!<    Instance of the mpi wrapper class
{
    ...
    for(iSize_t v=0;v<=gridMax;++v)
    {
        ...
        this->CalculateBandstructure(lowerBand,secondBand,bandWidth,bandGap,true,&dummy,false,dimX,dimY,offsetX,offsetY,&currentHamiltonian,mpi);
    ...
    if(0 == mpi.m_id)	// FOR THE MASTER NODE
	{
        if((*optionList)["plot-band-width"].as<bool>())
        {
            ...
            //  Write a python script to automatically plot the band width vs. V0 parameter
...
void SingleParticleHamiltonianArray::PlotMagnetization(
    boost::program_options::variables_map* optionList,  
                                            //!<    Command line arguments
    utilities::MpiWrapper& mpi)             //!<    Instance of the mpi wrapper class
{
    ...
    this->CalculateBandstructure(&dummy,&dummy,dummy,dummy,false,magnetization,
    true,dimX,dimY,offsetX,offsetY,&currentHamiltonian,mpi);
...
void SingleParticleHamiltonianArray::GenerateWannierCoefficients(
                                    ...
                                    //!<    Instance of the mpi wrapper class
{
    ...
    //  In this implementation we have epsilon_alpha = 0 (in the paper's notation)
    
    //  A_x(kx,ky) = <kx,ky|X|kx+1,ky> 
    //  = Sum_{band index alpha} u_alpha^*(kx,ky) u_alpha(kx+1,ky)

    //  A_y(kx,ky) = <kx,ky|Y|kx,ky+1> 
    //  = Sum_{band index alpha} u_alpha^*(kx,ky) u_alpha(kx,ky+1)
    
    dcmplx* Ax = new dcmplx[dimX*dimY];
    //dcmplx* Ay = new dcmplx[dimX*dimY];
    
    //  The bloch coefficients are stored in the array blochTable. The table
    //  is indexed as kx*dimY + ky

    //  Now, for each kx,ky, generate the sum over the band index

    for(kState_t kx=0;kx<dimX;++kx)
    {
        for(kState_t ky=0;ky<dimY;++ky)
        {
            //  Impose periodic boundary conditions in kx and ky
            ...
            //dcmplx aTempY = utilities::linearAlgebra::DotProduct<dcmplx>(blochTable[kx*dimY+ky].m_blochCoefficients,
            //                blochTable[kx*dimY+kyPlusOnePeriodic].m_blochCoefficients,blochTable->m_dim);

            //  Set the Berry connection to the normalized accumulated value
            //  as defined in Eq. 27:
            
            Ax[kx*dimY+ky] = aTempX/abs(aTempX);
            //Ay[kx*dimY+ky] = aTempY/abs(aTempY);
            
            //std::cout<<"\n\tAx["<<kx*dimY+ky<<"] IS "<<Ax[kx*dimY+ky]<<std::endl;
        }
    }
    
    //////  Tabulate some coefficients that we will need        ////////////////////
    
    //  As defined in Eq. 30:
    
    //  lambdaX(ky) = [product_{kx=0}^{dimX-1} Ax(kx,ky)]^(1/N_x),
    ...
    for(kState_t kx=0;kx<dimX;++kx)
    {
        for(kState_t ky=0;ky<dimY;++ky)
        {
            //std::cout<<"\n\tLAMBDA["<<ky<<"] WAS "<<lambdaX[ky]<<std::endl;
        
            lambdaX[ky] *= Ax[kx*dimY+ky];
            
            //std::cout<<"\n\tLAMBDA["<<ky<<"] NOW IS "<<lambdaX[ky]<<std::endl;
        }
    }
    
    //  Find the appropriate root of those products
    
    for(kState_t ky=0;ky<dimY;++ky)
    {
        lambdaX[ky] = std::pow(lambdaX[ky],1.0/dcmplx(dimX,0));
        
        //std::cout<<"\n\tLAMBDA["<<ky<<"] IS "<<lambdaX[ky]<<std::endl;
    }
    ...
    //  For kx=0 we have a product from kappa=0 to -1. Let's define that
    //  to be 1.0
    ...
    //  For kx>=1, we start generating the product from Ax[kx=0,ky]
    ...
    //  lambdaY(0) = [product_{ky=0}^{dimY-1} Ay(0,ky)]^(1/N_y),
    
    //  Generate product of Ay[0] values
    ...
    //  Find the appropriate root of the product
    
    lambdaY = std::pow(lambdaY,1.0/(double)dimY);
    
    //////  Tabulate U(ky) as defined in Eq 53      ////////////////////////////////

    //  Setting their parameter C to be +1 we have:
    
    //  U (non normalized) = 1/dimX sum_kx {[aProd(kx,ky) Ay(kx,ky)]/[aProd(kx,ky+1) Ay(0,ky)]}
    //                                     /[lambdaX(ky)/lambdaX(ky+1)]^kx
    ...
    for(kState_t kx=0;kx<dimX;++kx)
    {
        for(kState_t ky=0;ky<dimY;++ky)
        {
            //  Impose periodic boundary conditions in ky
    ...
    //  phi_y(x,ky) are fixed by the argument given in section IV B of the paper.
    //  It will end up being independent of x
    
    //  By definition, the 0,0 phase factor is set to 0 (so exp(phase) = 1.0). 
    //  The remaining factors are calculated from Eq 63:
    //
    //  e^{I*phi(ky_adjacent)} = e^{I*phi(ky)} lambda_y(0)/A_y(0,ky) omega_y / U_y(k_y)
    ...
    for(kState_t ky=0;ky<dimY-1;++ky)
    {
        ...
        //std::cout<<"\n\tphaseFactors["<<ky+1<<"] = "<<phaseFactors[ky+1]<<std::endl;
    }
            ...
            for(kState_t kx=0;kx<dimX;++kx)
            {
                for(kState_t ky1=0;ky1<dimY;++ky1)
                {
                    ...
                    if(ky == ky1)
                    {
                        ...
                        //std::cout<<"\tM["<<index1<<","<<index2<<"]  = "<<M[index1*dim+index2]<<std::endl;
                    }
                    else
                    {
    ...
    //  Test unitarity of the change of basis transformation
    
    std::cout<<std::endl;

    //  Multiply M and Mdagger
    ...
    for(int i=0;i<dim;++i)
    {
        for(int j=0;j<dim;++j)
        {
            dcmplx productMatrix = 0.0;
        
            for(int k=0;k<dim;++k)
            {
                productMatrix += M[i*dim+k]*std::conj(M[j*dim+k]);
            }
            
            std::cout<<"\tM*Mdagger["<<i<<","<<j<<"]  = "<<productMatrix<<std::endl;
        }
    }
...
void SingleParticleHamiltonianArray::CalcualteSigmaZMap(
    const iSize_t dimX,             //!<    X-dimension of k-space grid
    const iSize_t dimY,             //!<    Y-dimension of k-space grid
    SingleParticleHamiltonian* blochTable,
                                    //!<    Table of Bloch wave functions
...
void SingleParticleHamiltonianArray::CalculateSpatialWaveFunctions(
    boost::program_options::variables_map* optionList,
                                        //!<    Command line arguments
    utilities::MpiWrapper& mpi)         //!<    Instance of the mpi wrapper class
{

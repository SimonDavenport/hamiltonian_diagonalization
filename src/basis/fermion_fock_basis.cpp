        ...
        const
    {
        if(m_basisStored)
        {
            ...
            if(_SEARCH_ERROR_CODE_ == tempIndex)
            {
                ...
                //std::cerr<<"STATE WAS: "<<std::bitset<64>(state)<<std::endl;
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
        ...
        utilities::MpiWrapper& mpi) //!<    Instance of the mpi wrapper class
    {
        ...
        
        {
            ...
            mpi.Gather<fock_t>(allowedFockStates,sectorCounter,
            fockStateBuffer,sectorTotalDimension,0,mpi.m_comm,status);
        ...
        if(0 == mpi.m_id)	// FOR THE MASTER NODE
        {
            PRINT("allowedFockStates",fockStateBuffer,sectorTotalDimension);
        }
        PAR_PAUSE();
        #endif
